...
public class DailyEffortController {
    ...
    public ResponseEntity<ApiResponse<EffortEntryWrapperResponse>> getMyEffortLogs(@PathVariable Integer weekId, Principal principal) {

        String currentUserId = principal != null ? principal.getName() : null;

        return ResponseEntity.ok(new ApiResponse<>(dailyEffortService.getMyEffortEntries(weekId, currentUserId)));

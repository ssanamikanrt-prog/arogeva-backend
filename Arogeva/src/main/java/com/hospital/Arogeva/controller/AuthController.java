...
public class AuthController {
    ...
    @Operation(summary = "Get All Managers", description = "Fetches a list of all users who have the ROLE_MANAGER role")
    @GetMapping("/managers")
    public ResponseEntity<ApiResponse<List<UserResponse>>> getManagersForDropdown() {

        return ResponseEntity.ok(new ApiResponse<>(authService.getManagersForDropdown()));
    ...
    public ResponseEntity<ApiResponse<LoginResponse>> registerUser(
            @RequestParam AppRole roleName,
            @Valid @RequestBody CreateUserRequest request) {


        LoginResponse response = authService.createUser(request, roleName);
    ...
    public ResponseEntity<ApiResponse<LoginResponse>> refreshToken(@CookieValue(name = "refresh_token", required = false) String refreshToken) {
        ...
        if (!response.isSuccess()) {
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ApiResponse<>(response));
        }
        return ResponseEntity.ok(new ApiResponse<>(response));
